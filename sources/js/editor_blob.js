...
(function (app) {
  ...
  var _polyfills = [{
    ...
    test: HTMLCanvasElement.prototype.toBlob,
    file: "HTMLCanvasElement-toBlob.js"
  }];
  var _config = {
    colors: [
  ...
  function save () {
    ...
    Utils.setSpinner(true);
    _saveDrawInfo(_saveToLocal);

    // if (Socket.isConnected()) {
    //   _saveDrawInfo(_saveToServer);
    // } else {
    //   _saveDrawInfo(_saveToDashboard);
    // }

  }

  function _saveDrawInfo (callback) {
    ...
    tempCanvas.toBlob(callback);
    tempCanvas = undefined;

  }

  function _saveToDashboard (blob) {

    _currentFakeId += 10;
    _savedDraw.globalID = _currentFakeId.toString();
    if (blob) {
      _savedDraw.blob = blob;
    }
    __saveToDashboard();
  ...
  function _saveToLocal (blob) {

    if (_savedDraw.minX >= 0) {
      _savedDraw.blobBig = blob;
      debugger;
  ...
  function _draft () {
    ...
    if (_changedAfterDraft) {
      ...
      _saveDrawInfo();
      Folder.saveDraft(_savedDraw, _setLocalDbDrawId);
    }
  ...
  function show (preloadedDraw) {
    ...
    if (preloadedDraw) {
      ...
      img.src = preloadedDraw.localUrlBig;
    } else {
